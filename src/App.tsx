...
const App = () => {
  ...
  const pasteTileNotation = (text: string, anchor = pasteAnchorRef.current) => {
    ...
    const matches = [...normalized.matchAll(/([0-9]+)([mpsz])/g)]
    ...
    const characterTiles = [...normalized].map((character) => {
      ...
      const souRank = '123456789'.indexOf(character)
      if (souRank >= 0) return `sou${souRank + 1}`
      return ({ 東: 'ton', 南: 'nan', 西: 'sha', 北: 'pei', 白: 'haku', 發: 'hatsu', 発: 'hatsu', 中: 'chun' } as Record<string, string>)[character] ?? null
    })
    const tileIds = matches.length && matches.map((match) => match[0]).join('') === normalized ? notationTiles : characterTiles
