...
const announcements = [
  { date: '2026年9月13日 02:27', title: '文字編集欄の区切り線の重なりを修正', text: 'ホームタブでカスタム色を追加したときに、文字背景色パレットと図形・線の区切り線が重なる問題を修正しました。色の数に合わせて文字編集欄の幅が広がります。' },
  { date: '2026年9月6日 00:06', title: '？牌を字牌一覧に追加', text: '牌一覧の字牌に「？」牌を追加しました。レイアウト上のワイルドカードとして配置・移動・回転・保存でき、牌理・受け入れの解析対象には含めません。' },
  { date: '2026年8月31日 00:49', title: '隠した要素のドラッグ操作を改善', text: '隠し表示中の要素や文字部分を、クリックで表示しなくてもそのままドラッグして移動できるようになりました。クリックは表示・再非表示、ドラッグは移動として動作します。' },
  { date: '2026年8月31日 00:26', title: '隠し表示をホームタブから操作可能に', text: '選択した要素をホームタブの「隠す」ボタンから非表示・解除できるようになりました。右クリックメニューからの操作も引き続き利用できます。' },
  { date: '2026年8月30日 22:35', title: '隠し表示に対応', text: '選択した文字の範囲や配置した要素を黒塗りで非表示にし、クリックまたはEnter／Spaceで表示できるようになりました。表示後にもう一度クリックすると再び隠せます。文字の範囲は書式メニュー、要素全体はホームタブまたは右クリックメニューから設定できます。' },
  { date: '2026年8月18日 15:13', title: 'マーカーの直線スナップを改善', text: 'Shiftを押しながら描くマーカーにも直線ツールと同じ軸・グリッドへの吸着を適用しました。通常のマーカー描画はこれまでどおり自由線として使えます。' },
  { date: '2026年8月18日 12:16', title: '牌理・受け入れの表示を整理', text: '選択打牌ごとのカードを見直し、シャンテン形と受け入れ枚数を分けて表示するようにしました。カード幅も確保し、詰まりにくい列数で表示します。' },
  { date: '2026年8月18日 12:06', title: '期待値計算のドラ表示牌を拡張', text: '期待値計算でドラ表示牌を最大5枚まで指定できるようになりました。カンによる複数のドラ表示牌にも対応しています。' },
  { date: '2026年8月18日 00:45', title: '描画と文字の書式を拡張', text: 'マーカーの色を図形・線の色設定から変更できるようにし、Shiftを押しながら描くと直線になります。長方形にも線のパターンが反映され、文字には下線と背景色を設定できます。' },
