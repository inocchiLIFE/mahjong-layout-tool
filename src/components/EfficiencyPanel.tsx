  ...
  return (
    <aside ref={panelRef} className="efficiency-panel" aria-live="polite">
      ...
      {discards.length > 0 && <div className="efficiency-discards"><div className="efficiency-discard-heading"><strong>選択打牌ごとの受け入れ</strong><span>ドラッグで並べ替え・不要な候補は非表示にできます。</span>{hiddenDiscardIds.length > 0 && <button type="button" className="discard-restore-button" onClick={() => setHiddenDiscardIds([])}>非表示を戻す（{hiddenDiscardIds.length}）</button>}</div>{visibleDiscards.map(({ discardTileId, result: discardResult }) => {
        ...
        return <div className="efficiency-discard" key={discardTileId} draggable onDragStart={(event) => startDiscardDrag(event, discardTileId)} onDragOver={(event) => event.preventDefault()} onDrop={() => reorderDiscard(discardTileId)} onDragEnd={() => setDraggedDiscardId(null)}>
          <div className="efficiency-discard-summary">
            <span className="discard-drag-handle" title="ドラッグして並べ替え" aria-hidden="true">⠿</span>
            <Tile tileId={discardTileId} />
            <div className="efficiency-discard-main">
              <div className="efficiency-discard-status"><b>{formatShanten(discardResult.shanten)}</b>{type && <strong className="iishanten-type discard">{type}</strong>}</div>
              <em>{acceptanceLabel} {discardResult.effectiveTileIds.length}種 {discardResult.effectiveTileCount}牌</em>
            </div>
            <button type="button" className="discard-hide-button" onClick={() => setHiddenDiscardIds((current) => [...current, discardTileId])} aria-label={`${TILE_MAP.get(discardTileId)?.label ?? ''}切りを非表示`} title="この候補を非表示">×</button>
          </div>
          {showTenpaiShapeBreakdown && discardResult.shanten === 1 ? tenpaiShapeSummary(hand, discardResult.shanten) : discardResult.effectiveTileIds.length > 0 && <div className="efficiency-discard-waits" aria-label={`${TILE_MAP.get(discardTileId)?.label ?? ''}切りの${acceptanceLabel}`}>{discardResult.effectiveTileIds.map((tileId) => <Tile key={tileId} tileId={tileId} />)}</div>}
        </div>
      })}</div>}
